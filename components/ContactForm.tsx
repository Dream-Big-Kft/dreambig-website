...
const ContactForm = () => {
  ...
  const isSubmittingRef = useRef(false); // prevents double-submit on fast clicks
  const formStartTrackedRef = useRef(false); // fires "Form Started" only once per session, not on every keystroke
  ...
  return (
    <form
      ...
      data-hj-suppress // prevents Hotjar session replay from recording personal data
    >
      <input
        type="hidden"
        name="_subject"
